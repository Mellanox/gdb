...
static void
arc_new_thread (struct lwp_info *lwp)
{
  int i;
...
static enum target_hw_bp_type Z_packet_to_hw_type (enum raw_bkpt_type type)
{
  switch (type) {
...
static int arc_low_insert_hbp (struct arc_debug_reg_state *state,
				enum raw_bkpt_type type_from_packet, CORE_ADDR addr, int len)
{
  int retval;
...
static int
arc_supports_z_point_type(char z_type)
{
	switch(z_type)
	{
	case Z_PACKET_HW_BP:
                  ...
                  int len, struct raw_breakpoint *bp)
{
  ...
  case raw_bkpt_type_read_wp:
  case raw_bkpt_type_access_wp:
    return arc_low_insert_hbp (&proc->priv->arch_private->debug_reg_state,
                               type, addr, len);
  default:
    /* Unsupported.  */
    return 1;
                  ...
                  int len, struct raw_breakpoint *bp)
{
  ...
  case raw_bkpt_type_read_wp:
  case raw_bkpt_type_access_wp:
    return arc_low_remove_hbp (&proc->priv->arch_private->debug_reg_state,
                               type, addr, len);
  default:
    /* Unsupported.  */
    return 1;
...
struct linux_target_ops the_low_target = {
  ...
  arc_breakpoint_at,
  arc_supports_z_point_type, /* supports_z_point_type */
  arc_insert_point, /* insert_point */
  arc_remove_point, /* remove_point */
  arc_stopped_by_watchpoint, /* stopped_by_watchpoint */
  arc_stopped_data_address, /* stopped_data_address */
  NULL, /* collect_ptrace_register */
  NULL, /* supply_ptrace_register */
  NULL, /* siginfo_fixup */
  arc_new_process, /* new_process */
  arc_new_thread, /* new_thread */
  NULL, /* new_fork */
  arc_prepare_to_resume, /* prepare_to_resume */
  /* process_qsupported */
...
struct linux_target_ops the_low_target = {
  ...
  .supports_z_point_type            = arc_supports_z_point_type,
